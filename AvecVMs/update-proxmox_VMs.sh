...
DEBIAN_FRONTEND=noninteractive apt update -o=Dpkg::Use-Pty=0 && DEBIAN_FRONTEND=noninteractive apt upgrade -y -o=Dpkg::Use-Pty=0 -y >> "$LOGFILE" 2>&1
...
for CTID in $(pct list | awk 'NR>1 {print $1}'); do
    ...
    if pct status "$CTID" | grep -q "running"; then
        ...
        pct exec "$CTID" -- bash -c "DEBIAN_FRONTEND=noninteractive apt update -o=Dpkg::Use-Pty=0 && DEBIAN_FRONTEND=noninteractive apt upgrade -y -o=Dpkg::Use-Pty=0 -y" >> "$LOGFILE" 2>&1
